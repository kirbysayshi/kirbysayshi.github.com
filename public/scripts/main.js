...
ksh.defaultInit = function(){
	...
	// This is really just a throwback. Used to be a whole jquery + canvas plugin!
	// Now it's all just built-in to the web platform.
	async function onContactClick(e) {
		e.preventDefault();
		e.stopPropagation();

		const footer = document.querySelector("#footer-container footer");
		const animIn = footer.animate(
			[
	...
	document
		.querySelector("#social-contacts-header .header-contact")
