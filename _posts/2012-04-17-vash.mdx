...
```jsx
@* A, all on one line *@
<div className="how"> @for(var i = 0; i < 1; i++){ <div className="item-@i">I be an item!</div> } </div>

@* B, with line breaks *@
<div className="how">
@for(var i = 0; i < 1; i++){
    <div className="item-@i">I be an item!</div>
}
</div>
```
...
```js
var compiled = vash.compile(
  "<li>I'm a template, @model.name!</li>",
);
```
